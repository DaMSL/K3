...
LIBS="-L/cm/shared/apps/boost/1.5.8/lib -L$HOME/lib -L/cm/shared/apps/cuda/7.0/lib64"
...
dist/build/k3/k3 \
  ...
  compile \
    $CXX -l cpp \
    --cpp-flags="-DK3DEBUG -DBOOST_LOG_DYN_LINK -DYAS_SERIALIZE_BOOST_TYPES=1 -Iruntime/cpp/include/ -Iruntime/cpp/include/external ${BOOSTINCLUDE} ${BSLINCLUDE} ${THRUSTINCLUDE} ${K3_CXXFLAGS} ${LIBS} -lboost_serialization -lboost_system -lboost_regex -lboost_thread -lyaml-cpp -lpthread -lboost_log_setup -lboost_log -lboost_program_options -lcsvpp -lcuda -lnvrtc -O4" \
    -r runtime $@
