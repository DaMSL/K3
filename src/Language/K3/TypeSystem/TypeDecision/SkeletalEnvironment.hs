...
constructTypeForDecl ((lAtts,sAtts),decl) = do
  ...
  aP_c <- lift $ freshUVar $ TVarSourceOrigin s
  aP_f <- lift $ freshUVar $ TVarSourceOrigin s
  aP_s <- lift $ freshUVar $ TVarSourceOrigin s
  ...
  (a_H, cs_H) <- liftEither (AnnotationDepolarizationFailure s) $
                    depolarize sAttTs
