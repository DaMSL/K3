...
symbol :: Identifier -> Provenance -> Bool -> Bool -> Bool -> K3 Symbol
symbol i prov hasMove hasCopy hasWb = Node (Symbol {symIdent=i, symProv=prov, symHasMove=hasMove, symHasCopy=hasCopy, symHasWb=hasWb, symLambdaChoice=0} :@: []) []

lambda :: Identifier -> K3 Effect -> Maybe (K3 Symbol) -> K3 Symbol
lambda i eff subLam = replaceCh (symbol i (PLambda eff) False True False) $ maybeToList subLam
