...
annotationCArgsEnv program = map makeAnnotationEnvEntry annotationCArgsDecls
  where
    makeAnnotationEnvEntry (tag -> (DDataAnnotation name _ members)) = (name, (concatMap getCArgs members))
    ...
    annotationContainsCArgs (tag -> DDataAnnotation _ _ members) = any memberHasCArgs members
