...
ALL_TABLES = ["sentinel", "customer", "lineitem",
...
class K3Executor(mesos.interface.Executor):
    ...
    def launchTask(self, driver, task):
        ...
        def run_task():
            ...
            self.sendTaskStatus(driver, mesos_pb2.TASK_RUNNING,
                                "%s is RUNNING" % str(self.job_id))
            ...
            # NOTE: The following keys must appear in provided data from
            # dispatcher
            try:
                # peerParams = hostParams["peers"]
                globalpeers = hostParams["peers"]
                localpeers = hostParams['me']
                dataFilelist = [DataFile(d['path'], d['var'], d['policy'])
                                for d in hostParams['data'][0]]
            ...
            if "perf_profile" in hostParams:
                frequency = hostParams[
                    "perf_frequency"] if "perf_frequency" in hostParams else '10'
            ...
            if "resultVar" in hostParams:
                k3_cmd += " --result_path $MESOS_SANDBOX --result_var " + \
                    hostParams["resultVar"]
            ...
            if 'seq_files' in hostParams:
                ...
                if len(si) != len(localpeers):
                    logging.error(
                        "Invalid deployment: number of switch indexes does not match number of local peers")
                    logging.error("Switch indexes size: %d", len(si))
                    logging.error("Peers size:          %d", len(globalpeers))
                    self.sendTaskStatus(
                        driver, mesos_pb2.TASK_FAILED, "Invalid Mosaic Deployment (switch indexes != peers)")
                    return
                mosaicSeqfile = SeqFile(ns, dd, si, tb)
                logging.debug('SEQ File Data: ')
                logging.debug('   Num switches:   %s',
                              str(mosaicSeqfile.num_switches))
                logging.debug('   Data Dir    :   %s',
                              str(mosaicSeqfile.data_dir))
                logging.debug('   Switch Indexex:   %s',
                              str(mosaicSeqfile.switch_indexes))
                logging.debug('   Tables        :   %s',
                              str(mosaicSeqfile.tables))
            else:
                mosaicSeqfile = None
            ...
            for dataFile in dataFilelist:
                ...
                try:
                    srcfiles = sorted([os.path.join(dataFile.path, f)
                                       for f in os.listdir(dataFile.path)])
                        ...
                        for i, dfile in enumerate(srcfiles):
                            ...
                            if peerNum >= peerStart and peerNum <= peerEnd:
                                if dataFile.varName not in inputFiles[peerNum - peerStart]:
                                    inputFiles[
                                        peerNum - peerStart][dataFile.varName] = []
                                inputFiles[
                                    peerNum - peerStart][dataFile.varName].append(dfile)
                                logging.debug(
                                    '    local peer# %d  ---> %d', peerNum - peerStart, dfile)

                                myTotalFiles += 1

                    if dataFile.policy == 'replicate':
                        for i in range(len(inputFiles)):
                            inputFiles[i][dataFile.varName] = [
                                d for d in srcfiles]
                        myTotalFiles = len(srcfiles) * len(localpeers)

                    if dataFile.policy == 'pinned':
                        inputFiles[0] = {
                            dataFile.varName: [d for d in srcfiles]}
                        myTotalFiles = len(srcfiles)

                    logging.debug("  %s dir has %d total files",
                                  dataFile.path, myTotalFiles)

                except IOError as e:
                    logging.error(
                        "Failed to open local path: %s  (on %s)", dataFile.path, self.host)
                    self.sendTaskStatus(driver, mesos_pb2.TASK_FAILED,
                                        "%s FAILED. Error opening local file, %s" % (str(self.job_id), dataFile))

            logging.debug(
                "Input File Allocation (for total of %d files): ", myTotalFiles)
            ...
            if mosaicSeqfile is not None:
                ...
                switchToPeer = {}
                seqFileYamlByPeer = {i: []
                                     for i in mosaicSeqfile.switch_indexes}
                peerFilesList = {idx: {t: [] for t in ALL_TABLES}
                                 for idx in mosaicSeqfile.switch_indexes}

                for idx, si in enumerate(mosaicSeqfile.switch_indexes):
                    filename = os.path.join(mosaicSeqfile.data_dir, "mux", str(
                        mosaicSeqfile.num_switches), 'mux_' + str(si) + "_" + str(mosaicSeqfile.num_switches))
                ...
                for table in ALL_TABLES:
                    ...
                    if not os.path.exists(path):
                        self.sendTaskStatus(
                            driver, mesos_pb2.TASK_FAILED, "Data Dir does not exist")
                        return
                    ...
                    for i, filepath in enumerate(sorted(os.listdir(path))):
                        for sw_index in mosaicSeqfile.switch_indexes:
                            if table == "sentinel" or (i % mosaicSeqfile.num_switches) == sw_index:
                                peerFilesList[sw_index][table].append(
                                    {'path': os.path.join(path, filepath)})
                                logging.debug(
                                    'Switch:  %d  =>  %s', sw_index, filepath)

                for idx in mosaicSeqfile.switch_indexes:
                    for table in ALL_TABLES:
                        seqFileYamlByPeer[idx].append(
                            {'seq': peerFilesList[idx][table]})
            ...
            for ip in peerPort.keys():
                ...
                for port in peerPort[ip]:
                    peermastermap.append(
                        dict(key=[ip, port], value=[ip, peerPort[ip][0]]))
            masters = [{'addr': [ip, peerPort[ip][0]]}
                       for ip in peerPort.keys()]
        ...
        # Create one YAML file for each peer on this host
            for i, peer in enumerate(localpeers):
                ...
                peerglobals["local_peers"] = [hostParams['peers'][p]
                                              for p in range(peerStart, peerEnd + 1)]
                ...
                if mosaicOutpath is not None:
                    peerglobals[
                        mosaicOutpath.var] = mosaicOutpath.prefix + str(i) + mosaicOutpath.suffix

                # Mosaic seqfile stuff
                if mosaicSeqfile is not None:
                    peerglobals["seqfiles"] = seqFileYamlByPeer[
                        mosaicSeqfile.switch_indexes[i]]
                ...
                peerFileName = os.path.join(
                    os.getenv('MESOS_SANDBOX'), 'peers%d.yaml' % i)
            ...
            curl_output = post_curl + '-F "file=@stdout_' + \
                self.host + '" ' + archive_endpoint
