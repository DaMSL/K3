...
map<string,string> show_globals() {
    ...
    result["peer_aggs"] = ([] (_Map<R_key_value<int, _Map<R_key_value<string, double>>>> coll) {
        ostringstream oss;
        auto f = [&] (R_key_value<int, _Map<R_key_value<string, double>>> elem) {oss << "{" + ("key:" + to_string(elem.key) + "," + "value:" + ([] (_Map<R_key_value<string, double>> coll) {
            ostringstream oss;
    ...
    result["agg_vals"] = ([] (_Map<R_key_value<string, double>> coll) {
        ostringstream oss;
        auto f = [&] (R_key_value<string, double> elem) {oss << "{" + ("key:" + elem.key + "," + "value:" + to_string(elem.value) + "}") << ",";
        return unit_t();};
        coll.iterate(f);
...
int main(int argc,char** argv) {
    ...
    matchers["peer_aggs"] = [] (string _s) {do_patch(_s,peer_aggs);};
    matchers["agg_vals"] = [] (string _s) {do_patch(_s,agg_vals);};
