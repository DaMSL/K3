  ...
  public:
      ...
      K3::__time_context()  {
        ...
        dispatch_table[__finished_tid] = [this] (void* payload)   {
          finished(*(static_cast<double*>(payload)));
        };
        dispatch_table[__start_tid] = [this] (void* payload)   {
          start(*(static_cast<unit_t*>(payload)));
        };
        dispatch_table[__q6_local_tid] = [this] (void* payload)   {
          q6_local(*(static_cast<unit_t*>(payload)));
        };
      }
      static int __q6_local_tid;
      ...
      double q6_result;
      bool __q6_result_set__ = false;
      unit_t q6_local(unit_t _)  {
        auto d = std::make_shared<K3::ValDispatcher<double>>((lineitem.filter(std::move([this] (R_l_comments_l_commitdate_l_discount_l_extendedprice_l_linenumber_l_linestatus_l_orderkey_l_partkey_l_quantity_l_receiptdate_l_returnflag_l_shipdate_l_shipinstruct_l_shipmode_l_suppkey_l_tax<K3::base_string,
        K3::base_string, double, double, int, K3::base_string, int, int, double, K3::base_string,
        ...
        double> r) mutable  {
          ...
          K3::base_string, K3::base_string, K3::base_string, K3::base_string, int,
          double>& r) mutable  {
            return acc + (r.l_extendedprice) * (r.l_discount);
          };
        }), 0.0));
      ...
      unit_t finished(const double& finishArg)  {
        [this] (const double& x) mutable  {
          q6_result = q6_result + x;
          return unit_t {};
        }(std::move(finishArg));
        ...
        if (query_done_cnt == peers.size(unit_t {})) {
          {
            unit_t result;
            print(std::move(concat("Result: ", std::move(rtos(q6_result)))));
      ...
      unit_t ready(unit_t _)  {
        ...
        if (nodes_ready_cnt == peers.size(unit_t {})) {
          ...
          return peers.iterate(std::move([this] (const R_addr<Address>& x) mutable  {
            auto d = std::make_shared<K3::ValDispatcher<unit_t>>(unit_t {});
            __engine.send(x.addr, __q6_local_tid, d);
            return unit_t {};
          }));
        } else {
          return unit_t {};
      ...
      unit_t initDecls(unit_t _)  {
        if (!(__q6_result_set__)) {
          q6_result = 0.0;
          __q6_result_set__ = true;
        }
        if (!(__master_set__)) {
      ...
      std::map<std::string, std::string> __prettify()  {
        ...
        result["q6_result"] = K3::prettify_real(q6_result);
      ...
      void __patch(std::map<std::string, std::string> bindings)  {
        ...
        if (bindings.count("q6_result") > (0)) {
          do_patch(bindings["q6_result"], q6_result);
          __q6_result_set__ = true;
        }
...
int __global_context::__q6_local_tid;
...
int main(int argc, char** argv)  {
  __global_context::__q6_local_tid = 0;
  ...
  __k3_context::__trigger_names[__global_context::__q6_local_tid] = "q6_local";
  ...
  __k3_context::__clonable_dispatchers[__global_context::__finished_tid] = make_shared<ValDispatcher<double>>();
  __k3_context::__clonable_dispatchers[__global_context::__start_tid] = make_shared<ValDispatcher<unit_t>>();
  __k3_context::__clonable_dispatchers[__global_context::__q6_local_tid] = make_shared<ValDispatcher<unit_t>>();
  Options opt;
