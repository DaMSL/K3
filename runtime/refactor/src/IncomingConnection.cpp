...
IncomingConnection::IncomingConnection(asio::io_service& service, CodecFormat format) {
...
InternalIncomingConnection::InternalIncomingConnection(asio::io_service& service,
    CodecFormat format)
  : IncomingConnection(service, format) { }

void InternalIncomingConnection::receiveMessages(shared_ptr<MessageHandler> m_handler,
  ...
  (boost_error ec, size_t bytes) {
    ...
    if (bytes == m->networkHeaderSize()) {
      ...
      auto payload_callback = [this_shared, m, payload_buf, e_handler, m_handler]
      (boost_error ec, size_t bytes) {
        if (ec) {
          (*e_handler)(ec);
        }
        ...
        if (bytes == m->payload_length_) {
          ...
          pv = make_shared<PackedValue>(std::move(*payload_buf), this_shared->format_);
          m->setValue(pv);
          (*m_handler)(m);
...
void ExternalIncomingConnection::receiveMessages(shared_ptr<MessageHandler> m_handler,
    shared_ptr<ErrorHandler> e_handler) {
  shared_ptr<int> header_int = make_shared<int>(0);
  auto in_buffer = asio::buffer(reinterpret_cast<char *>(header_int.get()), sizeof(int));

  // Callback for when the network header has been read (payload_length)
  auto this_shared = shared_from_this();
  auto header_callback = [this_shared, header_int, in_buffer, m_handler, e_handler]
  (boost_error ec, size_t bytes) {
    if (ec) {
      (*e_handler)(ec);
    }
    if (bytes == sizeof(int)) {
      ...
      auto buffer = asio::buffer(payload_buf->data(), payload_buf->size());
      auto payload_callback = [this_shared, header_int, payload_buf, m_handler, e_handler] (boost_error ec, size_t bytes) {
        if (ec) {
          (*e_handler)(ec);
        }
        ...
        if (bytes == *header_int) {
          ...
          shared_ptr<PackedValue> pv;
          pv = make_shared<PackedValue>(std::move(*payload_buf), this_shared->format_);
          auto m = make_shared<Message>(this_shared->peer_addr_, this_shared->peer_addr_, this_shared->trigger_, pv);
          (*m_handler)(m);
