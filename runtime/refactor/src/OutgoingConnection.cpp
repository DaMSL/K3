...
OutgoingConnection::OutgoingConnection(const Address& addr,
  ...
  if (!connected_) {
    ...
    throw std::runtime_error("InternalOutgoingConnection connect(): failed to connect");
  }
}
...
bool OutgoingConnection::connected() {
  return connected_;
}
...
void InternalOutgoingConnection::writeLoop(shared_ptr<NetworkMessage> pm,
  ...
  asio::async_write(*this_shared->getSocket(), *buffers, this_shared->strand_->wrap(callback));
}

void ExternalOutgoingConnection::send(shared_ptr<PackedValue> pm,
                                      shared_ptr<ErrorHandler> e_handler) {
  if (!connected_) {
    throw std::runtime_error("ExternalOutgoingConnection send(): not connected");
  }
...
void ExternalOutgoingConnection::writeLoop(shared_ptr<PackedValue> pm,
  ...
  buffers->push_back(asio::const_buffer(reinterpret_cast<char *>(header.get()), sizeof(int)));
