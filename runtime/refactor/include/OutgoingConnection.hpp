...
class InternalOutgoingConnection : public OutgoingConnection, 
 ...
 protected:
  void writeLoop(shared_ptr<NetworkMessage> pm, shared_ptr<ErrorHandler> e_handler);

  std::queue<shared_ptr<NetworkMessage>> outbox_;
};

class ExternalOutgoingConnection : public OutgoingConnection, 
 ...
 protected:
  void writeLoop(shared_ptr<PackedValue> pm, shared_ptr<ErrorHandler> e_handler);
