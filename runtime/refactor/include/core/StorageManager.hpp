...
class StorageManager {
 public:
  ...
  void openFile(Address peer, Identifier id, std::string path, StorageFormat,
                CodecFormat, IOMode);
  ...
  vector<shared_ptr<PackedValue>> doReadBlock(Address peer, Identifier id,
                                              int max_blocksize);
...
// And a different subclass for StorageFormat::Binary (that reads/writes an int
// as a header indicating the size in bytes of a value) and StorageFormat::Text
// (that seperates values with a newline). Either way, read in the bytes
// corresponding to a single value to construct a PackedValue
