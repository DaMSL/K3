...
class DCDIStreamHandle : public IOHandle {
 public:
  DCDIStreamHandle(string path)
      : IOHandle(shared_ptr<FrameCodec>()),
        LogMT("DCDIStreamHandle"),
        currentFrame(0)
  {
    try {
      ...
      BOOST_LOG(*this) << "Failed to open DCD file " << path;
    }
  }
  ...
  bool hasRead() { return currentFrame < NFILE && dcdFile.good(); }
 ...
 private:
  ...
  int NPRIV;  // ICNTRL(2)  if restart, total number of frames before first
              // print
  ...
  int FROZAT;  // ICNTRL(9) is NATOM - NumberOfFreeAtoms : it is the number of
               // frozen (i.e. not moving atoms)
  int DELTA4;  // ICNTRL(10) timestep in AKMA units but stored as a 32 bits
               // integer !!!
