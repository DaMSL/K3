...
void runProgram(Options opt) {
  ...
  for (auto& s : configurations) {
    ...
    engine->configure(simulation, se, make_shared<DefaultMessageCodec>(),
                      log_level, log_path, result_var, result_path, queues);
  ...
  for (auto& t : engines) {
    ...
    shared_ptr<thread> thread = engine->forkEngine(
        make_shared<virtualizing_message_processor>(contexts));
    l.push_back(thread);
  }
