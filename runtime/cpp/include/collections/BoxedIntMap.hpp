#ifndef K3_BOXEDINTMAP
#define K3_BOXEDINTMAP
...
namespace YAML {
template <class R>
struct convert<K3::Libdynamic::IntMap<K3::Box<R>>> {
  static Node encode(const K3::Libdynamic::IntMap<K3::Box<R>>& c) {
    Node node;
    if (c.size(K3::unit_t{}) > 0) {
      for (auto& i : c) {
        node.push_back(convert<K3::Box<R>>::encode(i));
      }
    } else {
      node = YAML::Load("[]");
    }
  ...
  static bool decode(const Node& node, K3::Libdynamic::IntMap<K3::Box<R>>& c) {
    for (auto& i : node) {
      c.insert(i.as<K3::Box<R>>());
    }
    return true;
  }
};
...
template <class E>
struct convert<K3::Libdynamic::IntMap<K3::Box<E>>> {
  template <class Allocator>
    static Value encode(const K3::Libdynamic::IntMap<K3::Box<E>>& c, Allocator& al) {
    Value v;
    v.SetObject();
    ...
    inner.SetArray();
    for (const auto& e : c) {
      inner.PushBack(convert<K3::Box<E>>::encode(e, al), al);
    }
