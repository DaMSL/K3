...
namespace YAML {
template <class R>
struct convert<K3::Map<K3::Box<R>>> {
  static Node encode(const K3::Map<K3::Box<R>>& c) {
    Node node;
    auto container = c.getConstContainer();
    if (container.size() > 0) {
      for (auto i : container) {
        node.push_back(convert<K3::Box<R>>::encode(i.second));
      }
    } else {
      node = YAML::Load("[]");
    }
  ...
  static bool decode(const Node& node, K3::Map<K3::Box<R>>& c) {
    for (auto& i : node) {
...
template <class E>
struct convert<K3::Map<K3::Box<E>>> {
  template <class Allocator>
    static Value encode(const K3::Map<K3::Box<E>>& c, Allocator& al) {
    Value v;
    v.SetObject();
    ...
    for (const auto& e : c.getConstContainer()) {
      inner.PushBack(convert<K3::Box<E>>::encode(e.second, al), al);
    }
