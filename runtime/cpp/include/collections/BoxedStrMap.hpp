#ifndef K3_BOXEDSTRMAP
#define K3_BOXEDSTRMAP
...
namespace YAML {
template <class R>
struct convert<K3::Libdynamic::StrMap<K3::Box<R>>> {
  static Node encode(const K3::Libdynamic::StrMap<K3::Box<R>>& c) {
    Node node;
  ...
  static bool decode(const Node& node, K3::Libdynamic::StrMap<K3::Box<R>>& c) {
    for (auto& i : node) {
...
template <class E>
struct convert<K3::Libdynamic::StrMap<K3::Box<E>>> {
  template <class Allocator>
    static Value encode(const K3::Libdynamic::StrMap<K3::Box<E>>& c, Allocator& al) {
    Value v;
    v.SetObject();
